...
def Tregularize_Covmatr_analit(CC, Iind, Jind, Tind, obstimes, exclude=None, RcorH=150000, RcorV=100, RcorT=1, fname_boxes='Box_surfareas_with_centers.xlsx'):
    if exclude is None:
        exclude = [1, 2, 3, 49, 40]
    ...
    for ii in range(N):
        for jj in range(ii + 1, N):
            ...
            DistT = abs(float(obstimes[int(Tind[ii])]) - float(obstimes[int(Tind[jj])]))
            LocM[ii, jj] = np.exp(-DistH / RcorH) * np.exp(-DistV / RcorV) * np.exp(-DistT / RcorT)
